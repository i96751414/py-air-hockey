...
class Game:
    def __init__(self, width=MIN_WIDTH, height=MIN_HEIGHT, fps=60):
        """
        Game class.
        ...
        self.__height_r = self.__height / 4
        self.__width_r = self.__width / 20
    ...
    def _play(self, mode):
        ...
        while True:
            ...
            if mode == MODE_SINGLE_PLAYER:
                ...
                if self.__level == LEVEL_IMPOSSIBLE:
                    if self._is_right_direction(ball_angle):
                        d = (self.__x_r2 - x_ball) / math.cos(ball_angle)
            ...
            collision1 = rounded_rect_collided_with_circle((self.__x_r1, y_r1, self.__width_r, self.__height_r),
                                                           1, (x_ball, y_ball), self.__ball_radius)

            collision2 = rounded_rect_collided_with_circle((self.__x_r2, y_r2, self.__width_r, self.__height_r),
                                                           1, (x_ball, y_ball), self.__ball_radius)
            ...
            if collision1 is not None or collision2 is not None:
                if not self.__has_collided:
                    ...
                    self.__x_r1 + self.__width_r + self.__ball_radius < x_ball < self.__x_r2 - self.__ball_radius:
